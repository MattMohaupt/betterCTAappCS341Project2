...
def command9(dbConn):
    ...
    dbCursor.execute("SELECT Distinct(Stations.Station_Name), Stops.Latitude, Stops.Longitude FROM Stations JOIN Stops ON Stations.Station_ID = Stops.Station_ID WHERE Stops.Latitude >= ? AND Stops.Latitude <= ? AND Stops.Longitude >= ? AND Stops.Longitude <= ? ORDER BY Stations.Station_Name asc;", (corners[1], corners[0], corners[2], corners[3]))
...
def boundaries(lat, lon):
    ...
    deltalon = (1/51) #Each degree of longitude is approximately 51 miles at the chicago latitude level so we need 1/50th for the mile radius
